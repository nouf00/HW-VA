...
var createUnknownKeywordNode = () => f.createKeywordTypeNode(ts.SyntaxKind.UnknownKeyword);
...
var callGetType = (zod, identifier, options) => {
  let type = null;
  if (zod.getType)
    type = zod.getType(ts2, identifier, options);
  return type;
};
...
var zodToTsNode = (zod, identifier, store, options) => {
  ...
  switch (typeName) {
    case "ZodString":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.StringKeyword);
    case "ZodNumber":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.NumberKeyword);
    case "ZodBigInt":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.BigIntKeyword);
    case "ZodBoolean":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.BooleanKeyword);
    case "ZodDate":
      return f2.createTypeReferenceNode(f2.createIdentifier("Date"));
    case "ZodUndefined":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.UndefinedKeyword);
    case "ZodNull":
      return f2.createLiteralTypeNode(f2.createNull());
    case "ZodVoid":
      return f2.createUnionTypeNode([
        f2.createKeywordTypeNode(ts2.SyntaxKind.VoidKeyword),
        f2.createKeywordTypeNode(ts2.SyntaxKind.UndefinedKeyword)
      ]);
    case "ZodAny":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.AnyKeyword);
    case "ZodUnknown":
      return createUnknownKeywordNode();
    case "ZodNever":
      return f2.createKeywordTypeNode(ts2.SyntaxKind.NeverKeyword);
    case "ZodLazy": {
      if (!getTypeType)
    ...
    case "ZodObject": {
      ...
      const members = properties.map(([key, value]) => {
        ...
        const propertySignature = f2.createPropertySignature(void 0, getIdentifierOrStringLiteral(key), isOptional ? f2.createToken(ts2.SyntaxKind.QuestionToken) : void 0, type);
    ...
    case "ZodRecord": {
      ...
      const node = f2.createTypeLiteralNode([f2.createIndexSignature(void 0, void 0, [f2.createParameterDeclaration(void 0, void 0, void 0, f2.createIdentifier("x"), void 0, f2.createKeywordTypeNode(ts2.SyntaxKind.StringKeyword), void 0)], valueType)]);
      return node;
    }
    case "ZodMap": {
      const valueType = zodToTsNode(zod._def.valueType, ...otherArgs);
    ...
    case "ZodFunction": {
      ...
      argTypes.push(f2.createParameterDeclaration(void 0, void 0, f2.createToken(ts2.SyntaxKind.DotDotDotToken), f2.createIdentifier(`args_${argTypes.length}`), void 0, f2.createArrayTypeNode(createUnknownKeywordNode()), void 0));
    ...
    case "ZodDefault": {
      ...
      type.forEachChild((node) => {
        if (![ts2.SyntaxKind.UndefinedKeyword].includes(node.kind)) {
