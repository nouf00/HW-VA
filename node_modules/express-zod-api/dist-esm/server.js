...
import { isLoggerConfig, makeErrorFromAnything } from "./common-helpers.js";
import { createLogger } from "./logger.js";
import { defaultResultHandler, lastResortHandler } from "./result-handler.js";
import { initRouting } from "./routing.js";
import createHttpError from "http-errors";
export const createParserFailureHandler = (errorHandler, logger) => (error, request, response, next) => {
    if (!error) {
        return next();
    }
...
export const createNotFoundHandler = (errorHandler, logger) => (request, response) => {
...
export function attachRouting(config, routing) {
...
export function createServer(config, routing) {
    const logger = isLoggerConfig(config.logger)
        ? createLogger(config.logger)
        : config.logger;
    const app = express();
